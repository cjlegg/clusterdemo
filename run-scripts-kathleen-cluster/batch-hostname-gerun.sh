...
# this file is expected to be called with [qsub] batch-hostname-gerun.sh <shape_letter> on Katleen (shape-letter=(S|C)) 
...
{ for item in RESULTS_DIR ARGUMENTS_FILE SOURCE_IMAGE SHAPE PY_EXECUTABLE PY_EXECUTABLE_ARGUMENTS
do
    echo $item=${!item}
done 
